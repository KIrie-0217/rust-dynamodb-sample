...
```bash
dynamo-db-rust$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
dynamo-db-rust$ sam local start-api
dynamo-db-rust$ curl http://localhost:3000/
```
...
```bash
dynamo-db-rust$ sam logs -n HelloWorldFunction --stack-name dynamo-db-rust --tail
```
